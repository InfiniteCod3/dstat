...
const CustomExpirationPlugin_1 = require("./CustomExpirationPlugin");
...
class Prefetcher {
    constructor(config = { plugins: [] }) {
        ...
        // 'edgio-devtools-syn' message is received (ie: a Devtools client is enabled).
    ...
    /**
     * Creates a fallback route that ensures that all requests are handled by the service worker so that
     ...
     */
    route() {
        // TODO: Re-enable this once we support cache-manifest.js for non-EdgeJS deployments
        // this.addRoutesFromManifest()
     ...
     * 1. Devtools sends the 'edgio-devtools-syn' event and starts listening for 'edgio-devtools-ack' event
     * 2. Service worker receives the 'edgio-devtools-syn' event and sends the 'edgio-devtools-ack' event
     ...
     * 3. Devtools receives the 'edgio-devtools-ack' event, looks for its current page html request,
     *    which corresponds to that tab very first request, and loads requests past that one.
     * 4. Devtools starts listening for live 'edgio-devtools-request' events
     *
     * Notes:
     * - there is no 1-1 channel between window client and Service Worker, so all opened tab
     *   will see the 'edgio-devtools-ack' event. Only the ones in handshake phase will handle it.
     */
    ...
    createRoute(routeConfig) {
        ...
        if (method !== 'get') {
            this.createRoute({ ...routeConfig, method: { value: 'get' } });
        }
        ...
        (0, workbox_routing_1.registerRoute)(this.shouldIntercept(patternRegexp, isPartialPathRoute), new workbox_strategies_1.CacheFirst({
            ...
            matchOptions: {
                ignoreVary: true, // without this, prefetched responses will not be served from the cache during navigation
            },
            plugins,
        }), this.transformHTTPMethod(method) // HTTPMethods map directly from @edgio/core lowercase => @workbox uppercase
        );
    }
    /**
     * Transform method from HTTPMethod type to uppercase HTTPMethod type from workbox
     * the way typescript tolerates
     * @param method
     * @returns
     */
