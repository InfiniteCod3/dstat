...
class RequestContext {
    ...
    callIfMatched(rule, callback, index) {
        const matches = rule;
        // In the future, this should be replaced by RulesVisitor
        if (matches.if) {
            let conditionIndex = 0;
            while (conditionIndex < matches.if.length) {
                if (this.isTrue(matches.if[conditionIndex])) {
                    const thenIndex = conditionIndex + 1;
                    matches.if[thenIndex] && this.callIfMatched(matches.if[thenIndex], callback, index);
                    break;
                }
                else {
                    const elseIndex = conditionIndex + 2;
                    if (elseIndex === matches.if.length - 1) {
                        // If the condition is last, we know it's an 'else', so we apply features and exit.
                        this.callIfMatched(matches.if[elseIndex], callback, index);
                        break;
                    }
                    else {
            ...
            // Nested arrays are parsed the same way as a root rules are - all of them are applied.
            rule.forEach(rule => this.callIfMatched(rule, callback, index));
        }
        else {
            callback(rule, index);
        }
    ...
    isTrue(conditional) {
        ...
        if (operators.length !== 1) {
            ...
            // NOTE: Deployed edge-control matching works with both array of strings and numbers regardless the type,
            // that's why we are using toString() method to compare values for "IN" and "NOT_IN".
            // The console UI is generating edge-control only with string values.
            // Examples:
            //    "in": [{ response: "status_code" }, [200]]
            //    "in": [{ response: "status_code" }, ["200"]]
            if (!right.some((v) => v.toString() === (left === null || left === void 0 ? void 0 : left.toString())))
                return false;
        }
        else if (conditional[exports.NOT_IN]) {
            const left = this.eval(conditional[exports.NOT_IN][0]);
            const right = conditional[exports.NOT_IN][1];
