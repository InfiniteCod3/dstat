...
class ModEdgeFunctions extends Mod_1.default {
        ...
        for (let edgeFunction of edgeFunctions) {
            try {
                ...
                if (result.statusCode) {
                    ...
                    // Where there is an exception or context.respondWith() was not called, respond with a 534 "Project Error"
                    this.set534Response('Edge function must call context.respondWith(response); before completion.');
                }
            }
            catch (e) {
                // Unhandled exception in the edge function are caught by quickjs-wasm/quickjs-runtime/src/lib.rs at
                // this point in the CLI there is no additional information.
                this.set534Response(`${e}`);
            }
    ...
    async run(edgeFunctionPath, phase) {
        ...
        // Set path, bytecode and local environmnet variables into envVarMap
        let envVarMap = new Map();
        envVarMap.set(constants_1.EDGIO_EDGE_FUNCTION_ENV_VARIABLES.path, edgeFunctionPath);
        envVarMap.set(constants_1.EDGIO_EDGE_FUNCTION_ENV_VARIABLES.quickjsBytecodeBase64, bytecode);
        ...
        return await EdgeFunctionsManager_1.default.instance().runEdgeFunction(this.context, edgeFunctionPath, envVarMap, usrVarMap, httpVarMap, this.context.getRequest().rawBody);
    }
    toString() {
