...
class DeploymentBuilder {
    ...
    async build(options) {
        ...
        const routerPath = (0, paths_1.getRouterPath)(config);
    ...
    async createServerlessBundle({ serverless, connector }, router, options) {
        ...
        this.writeFileSync((0, path_1.join)(this.jsInternalDir, paths_1.STATIC_ASSET_MANIFEST_FILE_NAME), JSON.stringify(router.staticAssetManifest, null, '  '));
    ...
    async buildSources() {
        ...
        try {
            return { router: Router_1.default.load((0, path_1.join)(this.jsInternalDir, paths_1.ROUTES_FILE_NAME)) };
        }
        finally {
            process.chdir(cwd);
        }
    ...
    preparePackageJson() {
        ...
        // We're done if package.json was already copied with the build directory (so we don't overwrite it)
